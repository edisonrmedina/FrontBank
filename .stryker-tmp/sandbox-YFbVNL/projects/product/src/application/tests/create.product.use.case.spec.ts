...
describe('CreateProductUseCase (Vitest)', () => {
  ...
  it('should call handleError with correct error message', () => {
    const req: ICreateProductRequest = {
      id: 'ERR100',
      name: 'BadProduct',
      description: '',
    ...
    useCase.execute(req).subscribe({
      error: () => {
        expect(errorHandlingServiceMock.handleError).toHaveBeenCalledWith(
          mockError,
          `Product creation failed: BadProduct`
        );
      },
    });
  });

  it('should set loading to false at the end (success)', (done) => {
    const req = {
      id: 'FIN01',
      name: 'Test',
      description: '',
  ...
  it('should call addProduct with exact response data', () => {
    const req = {
      id: 'DATA01',
      name: 'ProductX',
      description: 'desc',
